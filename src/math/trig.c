...
fix64_t fix64_tan(fix64_t angle) {
    ...
    int neg_angle = (octant & 1) != 0; // flip input range for 1,3,5,7
    int neg_result = (octant & 2) != 0; // negate result for 2,3,6,7
    int recip_result = ((octant + 1) & 2) != 0; // 1/result for 1,2,5,6
    ...
    if (recip_result) {
        ...
        // fix(1/result + rounding) = fix((1+res*rnd)/res)
        int64_t num_hi = (1ull << (CHEBYSHEV_FRAC_BITS + FIX64_FRAC_BITS - 64));
        int64_t num_lo = result >> 1;

        result = neg_result ? -result : result;
        result = fix64_impl_div_i128_i64_sat(num_hi, num_lo, result);
    } else {
        result = chebyshev_tan_impl(norm_a); // Q1.62
        result = neg_result ? -result : result;
        // Round to Q1.32
