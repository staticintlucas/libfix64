...
fix64_t fix64_tan(fix64_t angle) {
    ...
    int64_t angle_hi;
    uint64_t angle_lo =
        fix64_impl_mul_i64_u64_i128(angle.repr, CHEBYSHEV_8_PI, &angle_hi); // Q33.94
    ...
    int64_t num, denom;
    if (angle_sum) {
        num = CHEBYSHEV_ONE - result; // Q1.62
        denom = CHEBYSHEV_ONE + result; // Q1.62
    } else {
        num = result; // Q1.62
        denom = CHEBYSHEV_ONE; // Q1.62
    }
